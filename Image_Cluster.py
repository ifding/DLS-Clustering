...
class clusGAN(object):
    ...
    def gen_interpolations(self):
        ...
        if self.sampler == 'one_hot':
            ...
            random_vector = 0.10 * np.random.randn(1, self.z_dim-self.num_classes)
            z_n = np.tile(random_vector, (batch_size,1))

            traversal_z = np.tile(np.linspace(-0.5, 0.5, num=interpolations), int(np.ceil(batch_size * 1.0 / interpolations)))
            e_k = np.eye(self.num_classes)
            #x = np.linspace(0.0, 1.0, num=interpolations)
            x = np.ones(interpolations)
            #z_c = [np.outer(x, e_k[0]) + np.outer(1-x, m) for m in e_k]
            z_c = [np.outer(x, m) for m in e_k]
            z_c = np.vstack(np.array(z_c))

            for m in range(self.z_dim-self.num_classes):
                z_n[:,m] = traversal_z
                bz = np.hstack((z_n, z_c))
                bx = self.sess.run(self.x_, feed_dict={self.z: bz})
                mode_bx = xs.data2img(bx)
                mode_bx = grid_transform(mode_bx, xs.shape)
                imsave('logs/{}/{}/{}_z{}_cyc{}_gen{}/traversal{}_samples.png'.format(self.data, self.model, self.sampler,
...
if __name__ == '__main__':
    ...
    cl_gan = clusGAN(g_net, d_net, enc_net, xs, zs, args.data, args.model, args.sampler,
                     num_classes, dim_gen, n_cat, batch_size, beta_cycle_gen, beta_cycle_label)
    if args.train == 'True':
        cl_gan.train(num_batches=150000)
    else:
