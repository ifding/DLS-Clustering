...
class clusGAN(object):
    def __init__(self, g_net, d_net, enc_net, x_sampler, z_sampler, data, model, sampler,
        ...
        #print(self.z_infer_gen)

        #print(self.z_enc_logits)
        ...
        #z_infer_gen_recon, z_infer_label_recon, z_infer_logits_recon = self.enc_net(recon_x)
        ...
        self.g_loss = tf.reduce_mean(self.d_) + \
                      ...
                      #self.beta_cycle_label * tf.reduce_mean(tf.square(self.z_infer_gen - z_infer_gen_new)) +\
                      #self.beta_cycle_label * tf.reduce_mean(tf.square(self.z_enc_logits - z_infer_logits_new)) +\
    ...
    def _eval_cluster(self, latent_rep, labels_true, timestamp, val):
        ...
        print(np.array2string(latent_rep[:10], separator=', '))
